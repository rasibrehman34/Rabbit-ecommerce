...
const FilterSidebar = () => {
  ...
  const handleFilterChange = (e) => {
    ...
    if (type === 'checkbox') {
      if (checked) {
        newFilters[name] = [...(newFilters[name] || []), value]; // [XS,S,XL]
      } else {
        ...
        <div className="flex flex-wrap gap-2">
          {colors.map((color) => (
            <button
              ...
              className={` tracking-tighter w-7 h-7 rounded-full border border-gray-300 cursor-pointer transition hover:scale-105 ${filters.color === color ? "ring-2 ring-blue-500" : ""}`}
